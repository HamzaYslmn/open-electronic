    ...
    >
      <ReadoutGrid
        items={[
          ...
          { label: 'Battery needed', value: `${solar.batteryAh.toFixed(2)} Ah`, note: <T k="{autonomyDays} days at {dod}% DoD" vars={{ autonomyDays, dod: (dod * 100).toFixed(0) }} /> },
      ...
      {solar.deficit && (
        <Warning
          text="The panel harvests {harvestWh} Wh against a {whPerDay} Wh load, so the battery only ever drains and the node dies once it is empty. You need at least {panelW} W just to break even, and realistically two to three times that so it can also recover from cloudy spells."
          vars={{
      ...
      {!solar.deficit && solar.daysToRecharge > autonomyDays && (
        <Warning
          text="The surplus is positive but thin: refilling an empty battery takes {daysToRecharge} days, longer than the {autonomyDays} days of autonomy it provides. After one bad week the node may never catch up. Oversize the panel rather than the battery."
          vars={{ daysToRecharge: solar.daysToRecharge.toFixed(1), autonomyDays }}
        />
      )}
      {peakSunHours > 4 && (
        <Warning
          text="Over 4 peak sun hours is a summer or low-latitude figure. Size on the worst month you expect to operate in, not the average: in northern Europe December can be under one peak sun hour, a factor of five below midsummer."
        />
      )}
